...
mmix.wasm mmix.js: $(MMIXLIB_C_SOURCES) $(SOURCES)
	$(EMSDK_ENV) && $(EMCC) $(SOURCES) $(MMIXLIB_C_SOURCES) $(CFLAGS) $(EXPORTED_FUNCTIONS) -s WASM=1 -s "EXTRA_EXPORTED_RUNTIME_METHODS=['ccall', 'cwrap', 'FS_createDataFile', 'FS']" -o mmix.js
